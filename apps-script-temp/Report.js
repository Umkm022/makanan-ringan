...
var ReportService = {
  ...
  _labaRugi: function(params) {
    ...
    var totalPendapatan = details.reduce(function(s, d) { return s + (d.subtotal || 0); }, 0);
    var totalHpp = details.reduce(function(s, d) { return s + ((d.hpp || 0) * (d.qty || 0)); }, 0);
    var totalLabaKotor = details.reduce(function(s, d) { return s + (d.laba || 0); }, 0);
    var totalBiaya = biaya.reduce(function(s, b) { return s + (b.jumlah || 0); }, 0);
    var totalKomisi = komisi.filter(function(k) { return k.status === 'READY' || k.status === 'PAID'; }).reduce(function(s, k) { return s + (k.nilai_komisi || 0); }, 0);
    var returCost = KasService && KasService.hitungTotalReturCost ? KasService.hitungTotalReturCost() : 0;
    var labaBersih = totalLabaKotor - totalBiaya - totalKomisi - returCost;
    return respond(true, '', [{
      pendapatan: totalPendapatan,
      hpp: totalHpp,
      laba_kotor: totalLabaKotor,
      biaya_operasional: totalBiaya,
      komisi: totalKomisi,
      retur_cost: returCost,
  ...
  _bukuKas: function(params) {
    if (typeof KasService === 'undefined' || !KasService.getMutasiRekening) {
      return respond(false, 'KasService not available', []);
    }
  ...
  _rekapKas: function(params) {
    if (typeof KasService === 'undefined' || !KasService.getRekapKas) {
      return respond(false, 'KasService not available', null);
    }
    return KasService.getRekapKas();
  }
};
